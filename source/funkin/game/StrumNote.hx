...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int) {
		...
		var arr:Array<FlxColor> = PlayState.isPixelStage
			? ClientPrefs.data.arrowRGBPixel[leData]
			: ClientPrefs.data.arrowRGB[leData];
	...
	public function reloadNote()
	{
		...
		final index:Int = Std.int(Math.abs(noteData)) % 4;

		if (PlayState.isPixelStage)
		{
			loadGraphic(Paths.image('pixelVariant/' + texture));
			width = width / 4;
			height = height / 5;
			loadGraphic(Paths.image('pixelVariant/' + texture), true, Std.int(width), Std.int(height));

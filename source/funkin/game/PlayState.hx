...
import funkin.game.cutscenes.VideoSprite;

import funkin.game.Note.EventNote;
...
import funkin.game.stages.StageWeek1;
import funkin.game.stages.Spooky;
import funkin.game.stages.Philly;
import funkin.game.stages.Limo;
import funkin.game.stages.Mall;
import funkin.game.stages.MallEvil;
import funkin.game.stages.School;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// "GLOBAL" SCRIPTS
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/globalScripts/')) /* changed the name from "scripts" to "globalScripts" to not get confused with the scripts folder in the "levels"
																								  folder */
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/levels/scripts/$songName/'))
			for (file in FileSystem.readDirectory(folder))
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		// different score text formats
		if(ClientPrefs.data.scoreTxtType == 'Vanilla')
		{
		...
		{
			var commaSeparated:Bool = true;
			scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false, commaSeparated)}' + ' | ' + (!instakillOnMiss ? 'Misses: ${songMisses}' : "") + ' | ' + 'Rating: ${str}\n';
		}
	...
	{
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, CoolUtil.boundTo(1 - (elapsed * 9 * playbackRate), 0, 1))));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, CoolUtil.boundTo(1 - (elapsed * 9 * playbackRate), 0, 1))));
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		{
			FlxG.sound.play(Paths.soundRandom('game/miss/missnote', 1, 3), FlxG.random.float(0.1, 0.2));
		}	
	}
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % 4 == 0 && ClientPrefs.data.camZooms)
			{

...
class PhillyStreets extends BaseStage
{
	...
	override function create()
	{
		...
		{
			var skyImage = Paths.image('stages/weekend1/phillyStreets/phillySkybox');
			...
			var phillySkyline:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillySkyline', -545, -273, 0.2, 0.2);
			add(phillySkyline);
			darkenable.push(phillySkyline);

			var phillyForegroundCity:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyForegroundCity', 625, 94, 0.3, 0.3);
			add(phillyForegroundCity);
			darkenable.push(phillyForegroundCity);
		}

		var phillyConstruction:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyConstruction', 1800, 364, 0.7, 1);
		add(phillyConstruction);
		darkenable.push(phillyConstruction);

		var phillyHighwayLights:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyHighwayLights', 284, 305, 1, 1);
		add(phillyHighwayLights);
		darkenable.push(phillyHighwayLights);

		if(!ClientPrefs.data.lowQuality)
		{
			var phillyHighwayLightsLightmap:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyHighwayLights_lightmap', 284, 305, 1, 1);
		...
		var phillyHighway:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyHighway', 139, 209, 1, 1);
		add(phillyHighway);
		darkenable.push(phillyHighway);

		if(!ClientPrefs.data.lowQuality)
		{
			var phillySmog:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillySmog', -6, 245, 0.8, 1);
			add(phillySmog);
			darkenable.push(phillySmog);

			for (i in 0...2)
			{
				var car:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyCars', 1200, 818, 0.9, 1, ['car1', 'car2', 'car3', 'car4'], false);
				add(car);
				switch(i)
				{
			...
			phillyTraffic = new BGSprite('stages/weekend1/phillyStreets/phillyTraffic', 1840, 608, 0.9, 1, ['redtogreen', 'greentored'], false);
			add(phillyTraffic);
			darkenable.push(phillyTraffic);

			var phillyTrafficLightmap:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyTraffic_lightmap', 1840, 608, 0.9, 1);
		...
		var phillyForeground:BGSprite = new BGSprite('stages/weekend1/phillyStreets/phillyForeground', 88, 317, 1, 1);
		...
		if(_song.gameOverSound == null || _song.gameOverSound.trim().length < 1) GameOverSubstate.deathSoundName = 'gameover/pico/fnf_loss_sfx-pico';
		...
		if (isStoryMode)
		{
			...
			{
				case 'darnell':
					if(!seenCutscene) setStartCallback(videoCutscene.bind('weekend1/darnellCutscene'));
				case '2hot':
	...
	override function createPost()
	{
		...
		spraycanPile = new BGSprite('stages/weekend1/SpraycanPile', 920, 1045, 1, 1);
		precache();
		add(spraycanPile);
	...
	function darnellCutscene()
	{
		...
		var cutsceneMusic:FlxSound = new FlxSound().loadEmbedded(Paths.music('cutscenes/darnellCanCutscene'));
		cutsceneMusic.looped = true;
		FlxG.sound.list.add(cutsceneMusic);

		var darnellLaugh:FlxSound = new FlxSound().loadEmbedded(Paths.sound('cutscenes/darnell_laugh'));
		darnellLaugh.volume = 0.6;
		FlxG.sound.list.add(darnellLaugh);

		var neneLaugh:FlxSound = new FlxSound().loadEmbedded(Paths.sound('cutscenes/nene_laugh'));
		...
		cutsceneHandler.timer(cutsceneDelay + 5.1, function() //pico fires at can
		{
			...
			FlxG.sound.play(Paths.soundRandom('game/weekend1/shots/shot', 1, 4));
	...
	function precache()
	{
		...
		function createCan()
		{
			...
			lightCanSnd.loadEmbedded(Paths.sound('game/weekend1/Darnell_Lighter'));
			
			kickCanSnd = new FlxSound();
			FlxG.sound.list.add(kickCanSnd);
			kickCanSnd.loadEmbedded(Paths.sound('game/weekend1/Kick_Can_UP'));

			kneeCanSnd = new FlxSound();
			FlxG.sound.list.add(kneeCanSnd);
			kneeCanSnd.loadEmbedded(Paths.sound('game/weekend1/Kick_Can_FORWARD'));
			didCreateCan = true;
		}
		...
		function precacheCasing()
		{
			...
			if(!ClientPrefs.data.lowQuality)
			{
				casingFrames = Paths.getSparrowAtlas('stages/weekend1/PicoBullet'); //precache
			...
			gunPrepSnd.loadEmbedded(Paths.sound('game/weekend1/Gun_Prep'));
		...
		for (noteType in noteTypes)
		{
			switch(noteType)
			{
				...
				case 'weekend-1-firegun':
					...
					bonkSnd.loadEmbedded(Paths.sound('game/weekend1/Pico_Bonk'));
			}
		}
		...
		for (i in 1...5)
			Paths.sound('game/weekend1/shots/shot$i');
	}
	...
	override function noteMiss(note:Note)
	{
		switch(note.noteType)
		{
			case 'weekend-1-firegun':
				...
				{
					GameOverSubstate.deathSoundName = 'gameover/pico/fnf_loss_sfx-pico-explode';
					GameOverSubstate.loopSoundName = 'gameover/pico/gameOverStart-pico-explode';
